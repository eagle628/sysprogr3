...
class Heatmapimage : 
    ...
    def __init__(self,map_image):
        ...
        self.image_size = self.original_map.shape[0:2]
        self.color_weight = np.zeros(self.image_size+(1,), np.uint8)
    ...
    def __gaussian_2d_matrix(self,pos,var):
        ...
        x = np.arange(-1 * self.image_size[1] /2, self.image_size[1] / 2, 1)
        y = np.arange(-1 * self.image_size[0] / 2, self.image_size[0] / 2, 1)
        X, Y = np.meshgrid(x, y)
        return np.vectorize(self.__gaussian_2d)(X,Y)
    
    def add_gaussian(self,pos,var,weight):
        pos[0] = pos[0] - self.image_size[1]/2
        pos[1] = pos[1] - self.image_size[0]/2
